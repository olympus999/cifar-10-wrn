...
def expand_conv(init, base, k, stride):
    ...
    shortcut  = BatchNormalization(axis=channel_axis)(init)
    shortcut  = Activation('relu')(shortcut)
    
    x = ZeroPadding2D((1, 1))(shortcut)
    x = Convolution2D(base * k, (3, 3), strides=stride, padding='valid', kernel_regularizer=l2(weight_decay))(x)

    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
    
    x = ZeroPadding2D((1, 1))(x)
    x = Convolution2D(base * k, (3, 3), strides=(1, 1), padding='valid', kernel_regularizer=l2(weight_decay))(x)
    
    # Add shortcut
    
    shortcut = Convolution2D(base * k, (1, 1), strides=stride, padding='same', kernel_regularizer=l2(weight_decay))(shortcut)
...
def conv_block(input, n, stride, k=1, dropout=0.0):
    ...
    x = Activation('relu')(x)
    x = Convolution2D(n * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay))(x)
...
def create_wide_residual_network(input_dim, nb_classes=100, N=2, k=1, dropout=0.0, verbose=1):
    ...
    x = Convolution2D(16, (3, 3), padding='same', kernel_regularizer=l2(weight_decay))(x)
